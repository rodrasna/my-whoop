...
// MARK: - SleepRespChartView
// Per-night respiratory-rate trend (breaths/min over time), bounded to a sleep
// session's window. The series comes from the server's /v1/resp-series endpoint,
// which derives breathing from respiratory sinus arrhythmia (RSA) in the
// beat-to-beat RR-interval series — pure signal processing, no calibration.
// Distinct from the raw resp-ADC daily aggregate. iOS 16-safe.

struct SleepRespChartView: View {
    ...
    private var respColor: Color { WH.Color.strainBlue }
    ...
    var body: some View {
        ...
        .navigationTitle("Respiración durante el sueño")
        ...
        if points.count < 2 {
            ...
            Chart {
                ForEach(points) { pt in
                    AreaMark(x: .value("Hora", pt.date), y: .value("rpm", pt.value))
                        .foregroundStyle(LinearGradient(
                            colors: [respColor.opacity(0.28), respColor.opacity(0.0)],
                            startPoint: .top, endPoint: .bottom))
                        .interpolationMethod(.catmullRom)
                }
                ForEach(points) { pt in
                    LineMark(x: .value("Hora", pt.date), y: .value("rpm", pt.value))
                        .foregroundStyle(respColor)
                        .lineStyle(StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round))
                        .interpolationMethod(.catmullRom)
                }
                if let sel = selected {
                    PointMark(x: .value("Hora", sel.date), y: .value("rpm", sel.value))
                        .foregroundStyle(respColor)
                        .symbolSize(100)
    ...
    private var noDataCard: some View {
        HStack {
            ...
            VStack(spacing: WH.Spacing.xs) {
                ...
                Text("Aún no hay suficientes latidos para estimar la respiración")
                    .font(WH.Font.caption)
                    .foregroundStyle(WH.Color.textSecondary)
    ...
    private func callout(_ pt: TrendPoint) -> some View {
        VStack(spacing: 2) {
            Text(String(format: "%.1f rpm", pt.value))
    ...
    private var footnote: some View {
        Text("Estimada de la variación del ritmo cardíaco con la respiración (arritmia sinusal respiratoria). Es una aproximación por procesado de señal, no una medición clínica.")
            .font(WH.Font.caption)
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = points.map(\.value)
        let lo = max(0, (vals.min() ?? 8) - 2)
        let hi = (vals.max() ?? 20) + 2
        return lo...hi
    }
    ...
    private func reload() async {
        isLoading = true
        points = await metrics.respSeries(fromEpoch: session.startTs, toEpoch: session.endTs)
        isLoading = false
    }
}
